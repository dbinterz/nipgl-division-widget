...
function lgw_drive_get_settings() {
    return get_option('lgw_drive', array(
        'enabled'              => false,
        'key_path'             => '',
        'root_folder_id'       => '',
        'oauth_client_id'      => '',
...
add_action('admin_init', 'lgw_drive_oauth_callback');
function lgw_drive_oauth_callback() {
    if (!isset($_GET['lgw_oauth_callback']) || !current_user_can('manage_options')) return;

    $code  = sanitize_text_field($_GET['code']  ?? '');
    $error = sanitize_text_field($_GET['error'] ?? '');

    if ($error) {
        add_settings_error('lgw_drive_oauth', 'oauth_error', 'Google OAuth error: ' . $error, 'error');
        return;
    }
    if (!$code) return;
    ...
    if (is_wp_error($response)) {
        add_settings_error('lgw_drive_oauth', 'oauth_error', 'Token exchange failed: ' . $response->get_error_message(), 'error');
        return;
    }
    ...
    if (empty($data['refresh_token'])) {
        ...
        add_settings_error('lgw_drive_oauth', 'oauth_error', 'OAuth failed: ' . $msg, 'error');
        return;
    }
 ...
 */
function lgw_safe_filename($str) {
...
function lgw_drive_save_scorecard($post_id, $is_edit = false) {
    ...
    $settings = lgw_drive_get_settings();
    $token    = lgw_drive_get_access_token();
    if (!$token) {
        lgw_drive_log($post_id, 'error', 'Could not obtain access token — check OAuth or service account settings');
        return;
    }

    // Folder path components — use full team name for folder, not club prefix
    $date      = $sc['date'] ?? '';
    $year      = lgw_drive_year_from_date($date);
    $division  = lgw_safe_filename($sc['division'] ?? 'Unknown Division');
    $home_team_folder = lgw_safe_filename($sc['home_team'] ?? 'Unknown');
    $away_team_folder = lgw_safe_filename($sc['away_team'] ?? 'Unknown');
    $root_id   = $settings['root_folder_id'];

    // Generate PDF
    $pdf_bytes = lgw_build_scorecard_pdf($sc, $post_id);

    // Base filename — YYYYMMDD HomeTeam v AwayTeam
    $safe_home = lgw_safe_filename($sc['home_team'] ?? 'Home');
    $safe_away = lgw_safe_filename($sc['away_team'] ?? 'Away');
    ...
    $photo_path  = !$is_edit ? get_post_meta($post_id, 'lgw_photo_path', true) : '';
 ...
 */
function lgw_drive_ensure_path($token, $root_id, $path) {
    $parent = $root_id;
    foreach ($path as $name) {
        $existing = lgw_drive_find_folder($token, $parent, $name);
        if ($existing) {
            $parent = $existing;
        } else {
            $new_id = lgw_drive_create_folder($token, $parent, $name);
...
function lgw_drive_find_folder($token, $parent_id, $name) {
...
function lgw_drive_create_folder($token, $parent_id, $name) {
    $body = json_encode(array(
        'name'     => $name,
        'mimeType' => 'application/vnd.google-apps.folder',
        'parents'  => array($parent_id),
    ));
    $res = lgw_drive_request($token, 'POST',
...
function lgw_drive_upload_file($token, $parent_id, $filename, $content, $mime, &$error = null) {
    $metadata = json_encode(array(
        'name'    => $filename,
        'parents' => array($parent_id),
    ));
    $boundary = 'lgw_' . md5(uniqid('', true));
    $body     = "--$boundary\r\n"
 ...
 */
function lgw_drive_versioned_name($token, $folder_id, $base_name, $force_version = false) {
    if (!$force_version) {
...
function lgw_drive_request($token, $method, $url, $extra_headers = array(), $body = null, &$error = null) {
    $args = array(
        'method'  => $method,
        'timeout' => 30,
        'headers' => array_merge(
    ...
    if (is_wp_error($response)) {
        ...
        error_log('LGW Drive WP error: ' . $error);
        return false;
    }
    ...
    if ($code >= 400) {
        ...
        error_log('LGW Drive API ' . $code . ': ' . $raw);
        return false;
    }
    return $data;
}
...
function lgw_drive_get_access_token($key_path = '') {
    $s = lgw_drive_get_settings();
    ...
    if (!empty($s['oauth_refresh_token']) && !empty($s['oauth_client_id']) && !empty($s['oauth_client_secret'])) {
        return lgw_drive_get_oauth_token($s['oauth_client_id'], $s['oauth_client_secret'], $s['oauth_refresh_token']);
    }

    // Fall back to service account JWT
    return lgw_drive_get_sa_token($key_path ?: ($s['key_path'] ?? ''));
}

function lgw_drive_get_oauth_token($client_id, $client_secret, $refresh_token) {
    $cached = get_transient('lgw_drive_oauth_token');
...
function lgw_drive_get_sa_token($key_path) {
    $cached = get_transient('lgw_drive_token');
    if ($cached) return $cached;
    ...
    $jwt = lgw_drive_build_jwt($key_data['client_email'], $key_data['private_key']);
    if (!$jwt) return false;
...
function lgw_drive_build_jwt($client_email, $private_key) {
    if (!function_exists('openssl_sign')) {
        error_log('LGW Drive: openssl_sign not available');
        return false;
    }
    $now     = time();
    ...
    if (!openssl_sign($input, $sig, $private_key, 'SHA256')) {
        error_log('LGW Drive: JWT signing failed — ' . openssl_error_string());
        return false;
    }
    return $input . '.' . lgw_drive_b64url($sig);
}

function lgw_drive_b64url($data) {
    return rtrim(strtr(base64_encode($data), '+/', '-_'), '=');
}

// ── Utility ───────────────────────────────────────────────────────────────────

function lgw_drive_year_from_date($date_str) {
    // dd/mm/yyyy
    if (preg_match('/(\d{4})$/', $date_str, $m)) return $m[1];
    return date('Y');
}

function lgw_drive_mime_from_ext($ext) {
    return array(
        'jpg'  => 'image/jpeg',
        'jpeg' => 'image/jpeg',
        'png'  => 'image/png',
...
function lgw_drive_log($post_id, $level, $message) {
    ...
    update_post_meta($post_id, 'lgw_drive_log', $log);
    if ($level === 'error') error_log('LGW Drive [' . $post_id . ']: ' . $message);
}
...
add_action('lgw_scorecard_confirmed', function($post_id) {
    lgw_drive_save_scorecard($post_id, false);
});

// Fires when admin saves an edit
add_action('lgw_scorecard_admin_edited', function($post_id) {
    lgw_drive_save_scorecard($post_id, true);
});
 ...
 */
function lgw_drive_key_dir() {
    $upload = wp_upload_dir();
    return trailingslashit($upload['basedir']) . 'lgw-private/';
}

/**
 * Ensure the key directory exists and is protected from web access.
 */
function lgw_drive_ensure_key_dir() {
    $dir = lgw_drive_key_dir();
    if (!file_exists($dir)) {
        wp_mkdir_p($dir);
    }
...
add_action('wp_ajax_lgw_drive_upload_key', 'lgw_ajax_drive_upload_key');
function lgw_ajax_drive_upload_key() {
    if (!current_user_can('manage_options')) wp_send_json_error('Unauthorized');
    check_ajax_referer('lgw_drive_upload_key', 'nonce');
    ...
    $dir      = lgw_drive_ensure_key_dir();
    $filename = 'lgw-service-account.json';
    $dest     = $dir . $filename;
...
function lgw_drive_settings_section() {
    $s          = lgw_drive_get_settings();
    $nonce_test = wp_create_nonce('lgw_drive_test');
    ...
    if (!empty($_GET['lgw_oauth_connected'])) {
    ...
    <tr>
        ...
        <td>
            <input type="text" name="lgw_drive_root_folder"
    ...
    <tr>
        ...
        <td style="padding-top:20px">
            <p class="description" style="margin:0 0 10px">
                ...
                Set the authorised redirect URI to: <code><?php echo esc_html(lgw_drive_oauth_redirect_uri()); ?></code>
            </p>
        </td>
    </tr>
    <tr>
        ...
        <td>
            <input type="text" name="lgw_oauth_client_id"
    ...
    <tr>
        ...
        <td>
            <input type="password" name="lgw_oauth_client_secret"
    ...
    <tr>
        ...
        <td>
            <?php if ($has_oauth): ?>
                ...
                <?php endif; ?>
                <button type="button" class="button" id="lgw-oauth-disconnect" style="margin-left:8px;color:#842029">
                    ✖ Disconnect
                </button>
    ...
    (function() {
        ...
        // Key file upload
        document.getElementById('lgw-key-file-input').addEventListener('change', function() {
            ...
            fetch(ajaxurl, {method:'POST', body:fd, credentials:'same-origin'})
                ...
                .then(function(r) {
                    if (r.success) {
                        ...
                        document.getElementById('lgw-key-current').innerHTML =
                            '✅ <strong>Key uploaded</strong><br>'
                        ...
                        document.getElementById('lgw-key-path-field').value = r.data.key_path;
                    } else {
        ...
        // Test connection
        document.getElementById('lgw-drive-test').addEventListener('click', function() {
            var btn = this, res = document.getElementById('lgw-drive-test-result');
            btn.disabled = true;
            res.textContent = 'Testing…';
            res.style.color = '#333';
            var fd = new FormData();
            fd.append('action', 'lgw_drive_test');
...
add_action('wp_ajax_lgw_drive_test', 'lgw_ajax_drive_test');
function lgw_ajax_drive_test() {
    if (!current_user_can('manage_options')) wp_send_json_error('Unauthorized');
    check_ajax_referer('lgw_drive_test', 'nonce');
    ...
    delete_transient('lgw_drive_token');
    $token = lgw_drive_get_access_token($s['key_path']);
...
// ── Persist Drive settings (called from lgw_save_settings) ─────────────────

function lgw_drive_save_settings() {
    $s = lgw_drive_get_settings();

    // Handle OAuth disconnect
    $disconnect = !empty($_POST['lgw_oauth_disconnect']);

    $new = array(
        'enabled'             => !empty($_POST['lgw_drive_enabled']),
        'key_path'            => sanitize_text_field($_POST['lgw_drive_key_path']       ?? $s['key_path']),
        'root_folder_id'      => sanitize_text_field($_POST['lgw_drive_root_folder']    ?? ''),
        'oauth_client_id'     => sanitize_text_field($_POST['lgw_oauth_client_id']      ?? $s['oauth_client_id'] ?? ''),
        'oauth_client_secret' => sanitize_text_field($_POST['lgw_oauth_client_secret']  ?? $s['oauth_client_secret'] ?? ''),
        // Preserve existing refresh token unless disconnecting
        'oauth_refresh_token' => $disconnect ? '' : ($s['oauth_refresh_token'] ?? ''),
        // Preserve other keys from lgw_drive option (e.g. sheets config)
        'sheets_enabled'      => $s['sheets_enabled'] ?? false,
        'sheets_id'           => $s['sheets_id']      ?? '',
        'sheets_tabs'         => $s['sheets_tabs']    ?? array(),
    );

    update_option('lgw_drive', $new);
    delete_transient('lgw_drive_token');
    delete_transient('lgw_drive_oauth_token');
}
