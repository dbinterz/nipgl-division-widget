...
(function () {
  ...
  var ajaxUrl    = (typeof nipglData     !== 'undefined' && nipglData.ajaxUrl)
                 ? nipglData.ajaxUrl
                 : (typeof nipglChampData  !== 'undefined' && nipglChampData.ajaxUrl)
                 ? nipglChampData.ajaxUrl
                 : '/wp-admin/admin-ajax.php';
  // Always prefer nipglChampData for badges (always set); fall back to nipglData
  // if the division widget happens to be on the same page.
  var badges     = (typeof nipglChampData !== 'undefined' && nipglChampData.badges)
                 ? nipglChampData.badges
                 : (typeof nipglData !== 'undefined' ? nipglData.badges     : {});
  var clubBadges = (typeof nipglChampData !== 'undefined' && nipglChampData.clubBadges)
                 ? nipglChampData.clubBadges
                 : (typeof nipglData !== 'undefined' ? nipglData.clubBadges : {});

  // ── Badge lookup (same logic as nipgl-widget.js) ─────────────────────────────
  function badgeImg(team, cls) {
    if (!team) return '';
    cls = cls || 'nipgl-champ-team-badge';
  ...
  function champBadge(entry) {
    ...
    if (badges[entry]) return '<img class="nipgl-champ-team-badge" src="' + badges[entry] + '" alt="">';
    ...
    for (var key in clubBadges) {
      if (key.toUpperCase() === clubUpper) {
        return '<img class="nipgl-champ-team-badge" src="' + clubBadges[key] + '" alt="">';
      }
    }
    // Prefix match
    var bestKey = '', bestImg = '';
    ...
    return bestImg ? '<img class="nipgl-champ-team-badge" src="' + bestImg + '" alt="">' : '';
  }

  function renderTeamRow(team, score, isWinner, isLoser, placeholder) {
    var cls = 'nipgl-champ-team';
    if (isWinner) cls += ' nipgl-champ-winner';
    if (isLoser)  cls += ' nipgl-champ-loser';
    var badge = team ? champBadge(team) : '';
    var isTbd = !team;
    var nameCls = 'nipgl-champ-team-name' + (isTbd ? ' tbd' : '');
    ...
    return '<div class="' + cls + '">'
      ...
      + (scoreStr !== '' ? '<span class="nipgl-champ-score">' + scoreStr + '</span>' : '')
      + '</div>';
  }
  ...
  function renderMatch(match) {
    ...
    var cls = 'nipgl-champ-match';
    if (match.bye)      cls += ' nipgl-champ-bye';
    if (!home && !away) cls += ' nipgl-champ-tbd';

    var gameNumHtml = match.game_num
      ? '<div class="nipgl-champ-game-num">Game ' + match.game_num + '</div>'
      : '';
  ...
  function renderBracket(wrap, data) {
    ...
    // ── Mobile tabs
    var tabsEl = qs('.nipgl-champ-tabs', wrap);
    var tabsInner = tabsEl ? qs('.nipgl-champ-tabs-inner', tabsEl) : null;
    if (tabsInner) {
      tabsInner.innerHTML = '';
      rounds.forEach(function (name, i) {
        var tab = document.createElement('div');
        tab.className = 'nipgl-champ-tab' + (i === 0 ? ' active' : '');
    ...
    // ── Bracket
    var bracketEl = qs('.nipgl-champ-bracket', wrap);
    if (!bracketEl) return;
    bracketEl.innerHTML = '';
    ...
    rounds.forEach(function (roundName, ri) {
      ...
      if (ri > 0) {
        ...
        connCol.className = 'nipgl-champ-connector-col';
        bracketEl.appendChild(connCol);
      }

      var roundEl = document.createElement('div');
      roundEl.className = 'nipgl-champ-round' + (isFinal ? ' nipgl-champ-round-final' : '') + (ri === 0 ? ' mobile-active' : '');
      roundEl.dataset.round = ri;

      var dateStr = dates[ri] ? '<span class="nipgl-champ-round-date">' + escHtml(dates[ri]) + '</span>' : '';
      roundEl.innerHTML = '<div class="nipgl-champ-round-header">' + escHtml(roundName) + dateStr + '</div>'
        + '<div class="nipgl-champ-round-slots"></div>';

      var slotsEl = qs('.nipgl-champ-round-slots', roundEl);
      roundMatches.forEach(function (match, mi) {
        ...
        var isAdmin = typeof nipglChampData !== 'undefined' && nipglChampData.isAdmin == 1;
        ...
        if (isAdmin && !match.bye && (match.home || match.away)) {
          ...
          card.classList.add('nipgl-champ-editable');
          ...
          card.classList.add('nipgl-champ-has-scorecard');
          card.addEventListener('click', function () {
    ...
    if (tabsInner) {
      tabsInner.addEventListener('click', function (e) {
        var tab = e.target.closest('.nipgl-champ-tab');
        if (!tab) return;
        var ri2 = parseInt(tab.dataset.round);
        qsa('.nipgl-champ-tab', tabsInner).forEach(function (t) { t.classList.toggle('active', t === tab); });
        qsa('.nipgl-champ-round', bracketEl).forEach(function (r) {
  ...
  function runDrawAnimation(pairs, onComplete, wrap) {
    ...
    overlay.className = 'nipgl-champ-draw-overlay';
    overlay.innerHTML = [
      '<div class="nipgl-champ-draw-title">🏆 Championship Draw</div>',
      '<div class="nipgl-champ-draw-subtitle">The draw is being made…</div>',
      '<div class="nipgl-champ-draw-reveal">',
        '<div class="nipgl-champ-draw-slot-label" id="nipgl-draw-slot-label">Match 1</div>',
        '<div class="nipgl-champ-draw-team" id="nipgl-draw-home"></div>',
        '<div class="nipgl-champ-draw-vs">vs</div>',
        '<div class="nipgl-champ-draw-team" id="nipgl-draw-away"></div>',
      '</div>',
      '<div class="nipgl-champ-draw-progress" id="nipgl-draw-progress">0 / ' + matchCount + ' drawn</div>',
      '<div class="nipgl-champ-draw-pairs" id="nipgl-draw-pairs"></div>',
      '<button class="nipgl-champ-draw-btn nipgl-champ-draw-skip-btn" id="nipgl-draw-skip">Skip to End</button>',
    ].join('');
    ...
    var speed   = (typeof nipglChampData !== 'undefined' && nipglChampData.drawSpeed) ? parseFloat(nipglChampData.drawSpeed) : 1.0;
    ...
    function addChip(pair) {
      ...
      chip.className = 'nipgl-champ-draw-pair-chip' + (pair.bye ? ' nipgl-champ-draw-bye-chip' : '');
    ...
    function advance() {
      if (idx < pairs.length) {
        ...
        var subtitleEl = qs('.nipgl-champ-draw-subtitle', overlay);
        var revealEl   = qs('.nipgl-champ-draw-reveal',   overlay);
    ...
    function showPair(i) {
      ...
      if (pair.type === 'header') {
        ...
        divider.className = 'nipgl-champ-draw-round-header';
        ...
        var nonceH = (typeof nipglChampData !== 'undefined') ? nipglChampData.champNonce : '';
        if (cupIdH && nonceH) {
          post('nipgl_champ_advance_cursor', { champ_id: cupIdH, nonce: nonceH, draw_token: drawToken || '', section: (wrap ? wrap.dataset.section || '0' : '0') }, function () {});
        }
      ...
      timer = setTimeout(function () {
        ...
        var cupId = document.querySelector('.nipgl-champ-wrap') ?
          document.querySelector('.nipgl-champ-wrap').dataset.champId : '';
        var nonce = (typeof nipglChampData !== 'undefined') ? nipglChampData.champNonce : '';
        if (cupId && nonce) {
          post('nipgl_champ_advance_cursor', {
            champ_id: cupId, nonce: nonce, draw_token: drawToken || '',
            section: (document.querySelector('.nipgl-champ-wrap') ? document.querySelector('.nipgl-champ-wrap').dataset.section || '0' : '0')
    ...
    skipBtn.addEventListener('click', function () {
      ...
      while (idx < pairs.length) {
        ...
        if (pair.type === 'header') {
          ...
          var nonceS = (typeof nipglChampData !== 'undefined') ? nipglChampData.champNonce : '';
          if (cupIdS && nonceS) {
            post('nipgl_champ_advance_cursor', { champ_id: cupIdS, nonce: nonceS, draw_token: drawToken || '', section: (wrap ? wrap.dataset.section || '0' : '0') }, function () {});
          }
          continue;
        }
        matchIdx++;
      ...
      var subtitleEl2 = qs('.nipgl-champ-draw-subtitle', overlay);
      var revealEl2   = qs('.nipgl-champ-draw-reveal',   overlay);
  ...
  function openScorecardViewer(card, match) {
    var existing = qs('.nipgl-champ-sc-modal');
    if (existing) existing.parentNode.removeChild(existing);

    var nonce  = (typeof nipglChampData !== 'undefined') ? nipglChampData.champNonce : '';
    var modal  = document.createElement('div');
    modal.className = 'nipgl-champ-sc-modal';
    modal.innerHTML =
      '<div class="nipgl-champ-sc-modal-box">' +
        '<div class="nipgl-champ-sc-modal-header">' +
          '<span class="nipgl-champ-sc-modal-title">' + escHtml(match.home) + ' v ' + escHtml(match.away) + '</span>' +
          '<button class="nipgl-champ-sc-modal-close" aria-label="Close">&times;</button>' +
        '</div>' +
        '<div class="nipgl-champ-sc-modal-body"><p class="nipgl-champ-sc-loading">Loading scorecard…</p></div>' +
      '</div>';
    document.body.appendChild(modal);

    qs('.nipgl-champ-sc-modal-close', modal).addEventListener('click', function () {
    ...
    post('nipgl_champ_get_scorecard', { home: match.home, away: match.away, nonce: nonce }, function (res) {
      var body = qs('.nipgl-champ-sc-modal-body', modal);
      if (!res.success) {
        body.innerHTML = '<p class="nipgl-champ-sc-none">No scorecard has been submitted for this match yet.</p>';
        return;
      }
      var sc = res.data.sc;
      var conf = res.data.confirmed_by ? '✅ Confirmed by ' + escHtml(res.data.confirmed_by) : '⏳ Awaiting confirmation';
      var html = '<div class="nipgl-champ-sc-summary">' +
        '<div class="nipgl-champ-sc-meta">' +
          (sc.venue ? '<span>' + escHtml(sc.venue) + '</span>' : '') +
          (sc.date  ? '<span>' + escHtml(sc.date)  + '</span>' : '') +
        '</div>' +
        '<div class="nipgl-champ-sc-conf">' + conf + '</div>' +
        '<table class="nipgl-champ-sc-table">' +
      ...
      (sc.rinks || []).forEach(function (rk) {
        ...
        html += '<tr>' +
          '<td class="nipgl-champ-sc-rink">Rink ' + escHtml(String(rk.rink)) + '</td>' +
          '<td class="nipgl-champ-sc-players">' + escHtml((rk.home_players || []).join(', ')) + '</td>' +
          '<td class="nipgl-champ-sc-score ' + (homeWin ? 'win' : '') + '">' + escHtml(String(hs)) + '</td>' +
          '<td class="nipgl-champ-sc-vs">v</td>' +
          '<td class="nipgl-champ-sc-score ' + (awayWin ? 'win' : '') + '">' + escHtml(String(as)) + '</td>' +
          '<td class="nipgl-champ-sc-players">' + escHtml((rk.away_players || []).join(', ')) + '</td>' +
          '</tr>';
      });
      html += '</tbody>' +
        '<tfoot><tr>' +
          '<td colspan="2" class="nipgl-champ-sc-total-lbl">Total</td>' +
          '<td class="nipgl-champ-sc-score ' + (parseFloat(sc.home_total) > parseFloat(sc.away_total) ? 'win' : '') + '">' + escHtml(String(sc.home_total ?? '-')) + '</td>' +
          '<td></td>' +
          '<td class="nipgl-champ-sc-score ' + (parseFloat(sc.away_total) > parseFloat(sc.home_total) ? 'win' : '') + '">' + escHtml(String(sc.away_total ?? '-')) + '</td>' +
          '<td class="nipgl-champ-sc-total-lbl">Total</td>' +
        '</tr></tfoot>' +
        '</table></div>';
      body.innerHTML = html;
    });
  ...
  function hideDrawButtons(wrap) {
    var actionsEl = qs('.nipgl-champ-header-actions', wrap);
    if (actionsEl) actionsEl.parentNode.removeChild(actionsEl);
    // Add print button
    var headerEl = qs('.nipgl-champ-header', wrap);
    if (headerEl && !qs('.nipgl-champ-post-draw-actions', wrap)) {
      var printDiv = document.createElement('div');
      printDiv.className = 'nipgl-champ-header-actions nipgl-champ-post-draw-actions';
      printDiv.innerHTML = '<button class="nipgl-champ-btn nipgl-champ-btn-ghost nipgl-champ-print-btn">\uD83D\uDDA8 Print Draw</button>';
      qs('.nipgl-champ-print-btn', printDiv).addEventListener('click', function () {
        // Ensure all rounds visible (not hidden by mobile tabs) before printing
        var rounds = qsa('.nipgl-champ-round', wrap);
  ...
  function openDrawLoginModal(wrap, onSuccess) {
    var existing = qs('.nipgl-champ-draw-login-modal');
    if (existing) existing.parentNode.removeChild(existing);

    var nonce = (typeof nipglChampData !== 'undefined') ? nipglChampData.champNonce : '';

    var modal = document.createElement('div');
    modal.className = 'nipgl-champ-draw-login-modal';
    modal.innerHTML =
      '<div class="nipgl-champ-draw-login-box">' +
        '<div class="nipgl-champ-draw-login-title">🔑 Draw Authentication</div>' +
        '<input class="nipgl-champ-draw-login-input" type="text" placeholder="Enter passphrase" ' +
               'autocomplete="off" autocapitalize="none" spellcheck="false">' +
        '<div class="nipgl-champ-draw-login-actions">' +
          '<button class="nipgl-champ-draw-login-submit">Unlock Draw</button>' +
          '<button class="nipgl-champ-draw-login-cancel">Cancel</button>' +
        '</div>' +
        '<div class="nipgl-champ-draw-login-msg"></div>' +
      '</div>';
    document.body.appendChild(modal);

    var input   = qs('.nipgl-champ-draw-login-input',  modal);
    var submitBtn = qs('.nipgl-champ-draw-login-submit', modal);
    var msgEl   = qs('.nipgl-champ-draw-login-msg',    modal);
    input.focus();
    ...
    function doAuth() {
      ...
      post('nipgl_champ_draw_auth', { nonce: nonce, passphrase: passphrase }, function (res) {
    ...
    qs('.nipgl-champ-draw-login-cancel', modal).addEventListener('click', function () {
      modal.parentNode.removeChild(modal);
    });
  ...
  function initAdminDraw(wrap) {
    ...
    var drawBtn = qs('.nipgl-champ-admin-draw-btn', wrap);
    var loginBtn = qs('.nipgl-champ-draw-login-btn', wrap);
    ...
    function performDraw() {
      ...
      var nonce = (drawBtn ? drawBtn.dataset.nonce : '') ||
                  (typeof nipglChampData !== 'undefined' ? nipglChampData.champNonce : '');
      ...
      post('nipgl_champ_perform_draw', { champ_id: cupId, nonce: nonce, draw_token: drawToken || '', section: section }, function (res) {
  ...
  function openScoreEntry(wrap, card, match, roundIdx, matchIdx) {
    ...
    var existing = qs('.nipgl-champ-score-popover');
    if (existing) existing.parentNode.removeChild(existing);

    var cupId = wrap.dataset.champId;
    var nonce = (typeof nipglChampData !== 'undefined') ? nipglChampData.scoreNonce : '';
    ...
    pop.className = 'nipgl-champ-score-popover';
    pop.innerHTML =
      '<div class="nipgl-champ-score-pop-title">Enter Score</div>' +
      '<div class="nipgl-champ-score-pop-row">' +
        '<span class="nipgl-champ-score-pop-name">' + escHtml(homeName) + '</span>' +
        '<input class="nipgl-champ-score-pop-input" id="nipgl-score-home" type="number" min="0" max="99" value="' + escHtml(String(hs)) + '" placeholder="–">' +
      '</div>' +
      '<div class="nipgl-champ-score-pop-row">' +
        '<span class="nipgl-champ-score-pop-name">' + escHtml(awayName) + '</span>' +
        '<input class="nipgl-champ-score-pop-input" id="nipgl-score-away" type="number" min="0" max="99" value="' + escHtml(String(as)) + '" placeholder="–">' +
      '</div>' +
      '<div class="nipgl-champ-score-pop-actions">' +
        '<button class="nipgl-champ-score-pop-save">Save</button>' +
        '<button class="nipgl-champ-score-pop-cancel">Cancel</button>' +
      '</div>' +
      '<div class="nipgl-champ-score-pop-msg"></div>';
    ...
    var msgEl     = qs('.nipgl-champ-score-pop-msg', pop);
    homeInput.focus();

    qs('.nipgl-champ-score-pop-cancel', pop).addEventListener('click', function () {
      pop.parentNode.removeChild(pop);
    });

    qs('.nipgl-champ-score-pop-save', pop).addEventListener('click', function () {
      var saveBtn = this;
      saveBtn.disabled = true;
      saveBtn.textContent = 'Saving…';
      var scoreSection = wrap.dataset.section || '0';
      post('nipgl_champ_save_score', {
  ...
  function updateStatus(wrap, msg) {
    var statusEl = qs('.nipgl-champ-status', wrap);
    if (!statusEl) return;
    var dot = qs('.nipgl-champ-status-dot', statusEl);
    var txt = qs('.nipgl-champ-status-text', statusEl);
  ...
  function startDrawPoll(wrap, lastVersion) {
    ...
    function revealNextPair() {
      ...
      if (!overlay || !document.body.contains(overlay)) {
        ...
        overlay.innerHTML = [
          '<div class="nipgl-champ-draw-title">🏆 Championship Draw</div>',
          '<div class="nipgl-champ-draw-subtitle">The draw is being made…</div>',
          '<div class="nipgl-champ-draw-reveal" id="nipgl-draw-reveal-v">',
            '<div class="nipgl-champ-draw-slot-label" id="nipgl-draw-slot-label-v">Match</div>',
            '<div class="nipgl-champ-draw-team" id="nipgl-draw-home-v"></div>',
            '<div class="nipgl-champ-draw-vs">vs</div>',
            '<div class="nipgl-champ-draw-team" id="nipgl-draw-away-v"></div>',
          '</div>',
          '<div class="nipgl-champ-draw-progress" id="nipgl-draw-progress-v">Waiting for draw to begin…</div>',
          '<div class="nipgl-champ-draw-eta" id="nipgl-draw-eta-v"></div>',
          '<div class="nipgl-champ-draw-pairs" id="nipgl-draw-pairs-v"></div>',
        ].join('');
        document.body.appendChild(overlay);
      }
      ...
      var speed  = (typeof nipglChampData !== 'undefined' && nipglChampData.drawSpeed)
                 ? parseFloat(nipglChampData.drawSpeed) : 1.0;
    ...
    function showViewerComplete(bracket) {
      ...
      if (overlay && document.body.contains(overlay)) {
        ...
        var existing = qs('.nipgl-champ-draw-skip-btn', overlay);
        if (existing) existing.parentNode.removeChild(existing);
        var closeBtn = document.createElement('button');
        closeBtn.className = 'nipgl-champ-draw-skip-btn';
    ...
    function updateProgress(cur, tot) {
      ...
      if (eta && tot > 0 && cur < tot) {
        var spd = (typeof nipglChampData !== 'undefined' && nipglChampData.drawSpeed)
                ? parseFloat(nipglChampData.drawSpeed) : 1.0;
    ...
    function doPoll() {
      ...
      post('nipgl_champ_poll', { champ_id: cupId, version: lastVersion, cursor: cursor, section: sectionId }, function (res) {
  ...
  function initChampWidget(wrap) {
    ...
    var printBtn = qs('.nipgl-champ-print-btn', wrap);
    if (printBtn) {
  ...
  function initSectionTabs(champId) {
    var outer = document.querySelector('.nipgl-champ-tabs-outer');
    if (!outer) return;
    var storageKey = 'nipgl_champ_section_' + champId;

    // Restore saved tab
    var saved = sessionStorage.getItem(storageKey);
    if (saved !== null) {
      var btn = outer.querySelector('.nipgl-champ-section-tab[data-section="' + saved + '"]');
      if (btn) {
        ...
        var pane = outer.querySelector('.nipgl-champ-section-pane[data-section="' + saved + '"]');
  ...
  document.addEventListener('DOMContentLoaded', function () {
    qsa('.nipgl-champ-wrap').forEach(initChampWidget);
    // Sticky section tabs — run after wraps are initialised
    var firstWrap = document.querySelector('.nipgl-champ-wrap');
