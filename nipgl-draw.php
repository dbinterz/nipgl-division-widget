 ...
 *
 * Public API:
 *   nipgl_draw_build_bracket( array $numbered_entries, array $options ) : array|null
 *   nipgl_draw_default_rounds( int $n ) : string[]
 *   nipgl_draw_cup_club( string $team_name ) : string
 *
 ...
 */
function nipgl_draw_cup_club( $team_name ) {
    $name     = trim( $team_name );
 ...
 */
function nipgl_draw_default_rounds( $n ) {
    $bracket_size = 1;
 ...
 * @return array|null
 */
function nipgl_draw_build_bracket( array $numbered, array $opts ) {
    $n = count( $numbered );
    if ( $n < 2 ) return null;

    // Resolve options
    $get_club        = $opts['get_club']        ?? 'nipgl_draw_cup_club';
    ...
    // ── Round names ────────────────────────────────────────────────────────────
    if ( $prelim_count > 0 ) {
        $main_rounds = nipgl_draw_default_rounds( $half );
